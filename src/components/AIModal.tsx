...
export const AIModal: React.FC<any> = ({ onClose, coreIdentity, habits, localData, targetBaseMode, appMode, currentMood }) => {
  ...
  const ask = async () => {
    ...
    const targetBaseModeContext = appMode === AppMode.TARGET 
      ? `\nCRITICAL DIRECTIVE (TARGET BASE MODE ACTIVE): The user is in a state of absolute hyper-focus. Their Grand Target is: "${targetBaseMode?.target || 'Unknown'}". Their deadline is: "${targetBaseMode?.deadline || 'Unknown'}". Their core pillars are: ${(targetBaseMode?.pillars || []).join(', ')}. EVERY piece of advice, motivation, or analysis you provide MUST be filtered through the lens of achieving this Grand Target. Be intense, uncompromising, and ruthlessly focused on this goal.`
      : '';

    const vacationModeContext = appMode === AppMode.VACATION
      ? `\nCRITICAL DIRECTIVE (GRACE/VACATION MODE ACTIVE): The user is currently in a recovery and rest phase. Prioritize their well-being, mental health, and relaxation. Do not push them to be productive. Encourage restorative activities and self-compassion.`
      : '';

    const moodContext = currentMood
      ? `\nCURRENT MOOD: The user is currently feeling ${currentMood}. Tailor your response to acknowledge and support this emotional state. If they are feeling negative (SAD, ANGRY, DEPRESSED), offer gentle wisdom or Nasiha (from Quran/Hadith if appropriate, or general philosophical wisdom) to comfort them. If they are positive (GRATEFUL, ENERGETIC), amplify their energy.`
      : '';
  ...
  return (
    <div className="space-y-6 animate-in w-full max-w-md mx-auto pb-12">
      <div className={`p-6 rounded-[1.5rem] text-white flex flex-col items-center gap-3 text-center shadow-2xl relative overflow-hidden ${appMode === AppMode.TARGET ? 'bg-red-600' : appMode === AppMode.VACATION ? 'bg-emerald-600' : 'bg-indigo-600'}`}>
        ...
        <Bot size={32}/>
        <h2 className="text-xl font-mono uppercase tracking-tighter">{appMode === AppMode.TARGET ? 'Oracle (Target Base Mode)' : appMode === AppMode.VACATION ? 'Oracle (Grace Mode)' : 'Oracle AI'}</h2>
